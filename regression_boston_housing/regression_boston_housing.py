...
# X = pd.DataFrame(boston[['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX', 'RM', 'AGE', 'DIS', 'RAD', 'TAX', 'PTRATIO', 'B', 'LSTAT']], columns=['CRIM', 'ZN', 'INDUS', 'CHAS', 'NOX', 'RM', 'AGE', 'DIS', 'RAD', 'TAX', 'PTRATIO', 'B', 'LSTAT'])
# Y = pd.DataFrame(boston["MEDV"], columns=["MEDV"])
...
def user_input_features():
    ...
    # 아 쒸벌~ df의 key index 문제가 아니라 streamlit이 min()과 max()를 처리할때 발생하는 문제였네...
    # https://github.com/dataprofessor/streamlit_freecodecamp/issues/1
...
shap_values = explainer.shap_values(X) # 여기서 np.bool deprecated라고 오류가 발생하는데 정확히 무슨 원인인지, 어떻게 해결할지 모르겠음

# 여러가지 방법으로 해봤는데 shap 자체에 문제가 있는 듯!
# import xgboost
